/* ═══════════════════════════════════════════════════
   U.E. Cascada Génesis — Interactive Scripts
   ═══════════════════════════════════════════════════ */

document.addEventListener('DOMContentLoaded', () => {

    // ── Hero Banner Slider ─────────────────────────
    const heroImages = document.querySelectorAll('.hero-slider .hero-image');
    const heroIndicators = document.querySelectorAll('.hero-slider-indicators .indicator');
    const heroSection = document.getElementById('hero');
    let currentSlide = 0;
    let slideTimeoutId;

    function getSlideInterval(slideIndex) {
        // Image 2 (index 1) and Image 3 (index 2) transition in 3 seconds as requested
        if (slideIndex === 1 || slideIndex === 2) {
            return 3000;
        }
        // Image 1 (index 0) remains active for 6 seconds
        return 6000;
    }

    function updateSlideState() {
        if (!heroSection) return;
        if (currentSlide !== 0) {
            heroSection.classList.add('hide-text');
        } else {
            heroSection.classList.remove('hide-text');
        }
    }

    function startSlideTimer() {
        clearTimeout(slideTimeoutId);
        const currentInterval = getSlideInterval(currentSlide);
        slideTimeoutId = setTimeout(nextSlide, currentInterval);
    }

    function nextSlide() {
        if (heroImages.length === 0) return;
        heroImages[currentSlide].classList.remove('active');
        if (heroIndicators.length > 0) {
            heroIndicators[currentSlide].classList.remove('active');
        }
        currentSlide = (currentSlide + 1) % heroImages.length;
        heroImages[currentSlide].classList.add('active');
        if (heroIndicators.length > 0) {
            heroIndicators[currentSlide].classList.add('active');
        }
        updateSlideState();
        startSlideTimer();
    }

    // Start the slider with the dynamic timer
    startSlideTimer();

    heroIndicators.forEach((indicator, index) => {
        indicator.addEventListener('click', () => {
            clearTimeout(slideTimeoutId);
            heroImages[currentSlide].classList.remove('active');
            if (heroIndicators.length > 0) {
                heroIndicators[currentSlide].classList.remove('active');
            }
            currentSlide = index;
            heroImages[currentSlide].classList.add('active');
            if (heroIndicators.length > 0) {
                heroIndicators[currentSlide].classList.add('active');
            }
            updateSlideState();
            startSlideTimer();
        });
    });

    // ── Preloader ──────────────────────────────────
    const preloader = document.getElementById('preloader');
    const preloaderStage1 = document.getElementById('preloaderStage1');
    const preloaderStage2 = document.getElementById('preloaderStage2');
    const preloaderFill = document.getElementById('preloaderFill');
    const preloaderPercent = document.getElementById('preloaderPercent');
    const preloaderCanvas = document.getElementById('preloaderParticles');
    const preloaderVideo = document.getElementById('preloaderVideo');
    const preloaderAudioToggle = document.getElementById('preloaderAudioToggle');
    const preloaderAudioIcon = document.getElementById('preloaderAudioIcon');
    const preloaderVideoProgressBar = document.getElementById('preloaderVideoProgressBar');
    const preloaderPlayOverlay = document.getElementById('preloaderPlayOverlay');
    const preloaderPlayBtn = document.getElementById('preloaderPlayBtn');
    const preloaderVideoCaption = document.getElementById('preloaderVideoCaption');
    const preloaderSkipBtn = document.getElementById('preloaderSkipBtn');

    // SVG Icons for Mute/Unmute
    const mutedSvg = `<svg viewBox="0 0 24 24" id="audioSvgIcon"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.21.05-.42.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>`;
    const unmutedSvg = `<svg viewBox="0 0 24 24" id="audioSvgIcon"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>`;

    let progress = 0;
    const stage1Duration = 3500; // time in ms for Stage 1 progress
    const stage1Start = performance.now();
    let isTransitioning = false;

    // Safety fallback timer IDs
    let stage2LoadFallback;

    // Floating particles on canvas (Stage 1)
    function initPreloaderParticles() {
        if (!preloaderCanvas) return null;
        const ctx = preloaderCanvas.getContext('2d');
        let w = preloaderCanvas.width = window.innerWidth;
        let h = preloaderCanvas.height = window.innerHeight;

        const particles = [];
        const particleCount = 40;

        for (let i = 0; i < particleCount; i++) {
            particles.push({
                x: Math.random() * w,
                y: Math.random() * h,
                r: Math.random() * 2 + 0.5,
                vx: (Math.random() - 0.5) * 0.5,
                vy: -Math.random() * 0.8 - 0.2,
                alpha: Math.random() * 0.5 + 0.1,
                color: ['rgba(54, 171, 204,', 'rgba(159, 219, 100,', 'rgba(235, 200, 79,'][Math.floor(Math.random() * 3)]
            });
        }

        let animId;
        function drawParticles() {
            ctx.clearRect(0, 0, w, h);
            particles.forEach(p => {
                ctx.beginPath();
                ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
                ctx.fillStyle = p.color + p.alpha + ')';
                ctx.fill();

                p.x += p.vx;
                p.y += p.vy;

                if (p.y < -10) { p.y = h + 10; p.x = Math.random() * w; }
                if (p.x < -10) p.x = w + 10;
                if (p.x > w + 10) p.x = -10;
            });

            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    const dx = particles[i].x - particles[j].x;
                    const dy = particles[i].y - particles[j].y;
                    const dist = Math.sqrt(dx * dx + dy * dy);
                    if (dist < 120) {
                        ctx.beginPath();
                        ctx.moveTo(particles[i].x, particles[i].y);
                        ctx.lineTo(particles[j].x, particles[j].y);
                        ctx.strokeStyle = 'rgba(54, 171, 204, ' + (0.08 * (1 - dist / 120)) + ')';
                        ctx.lineWidth = 0.5;
                        ctx.stroke();
                    }
                }
            }

            animId = requestAnimationFrame(drawParticles);
        }
        drawParticles();

        // Resize support
        const handleResize = () => {
            w = preloaderCanvas.width = window.innerWidth;
            h = preloaderCanvas.height = window.innerHeight;
        };
        window.addEventListener('resize', handleResize);

        return () => {
            cancelAnimationFrame(animId);
            window.removeEventListener('resize', handleResize);
        };
    }

    const cleanupParticles = initPreloaderParticles();

    // Stage 1 Progress Animation
    function updatePreloaderProgress(timestamp) {
        const elapsed = timestamp - stage1Start;
        const rawProgress = Math.min(elapsed / stage1Duration, 1);
        const easedProgress = 1 - Math.pow(1 - rawProgress, 3); // Ease-out cubic
        progress = Math.round(easedProgress * 100);

        if (preloaderFill) preloaderFill.style.width = progress + '%';
        if (preloaderPercent) preloaderPercent.textContent = progress + '%';

        if (rawProgress < 1) {
            requestAnimationFrame(updatePreloaderProgress);
        } else {
            // Once Stage 1 reaches 100%, trigger Transition to Stage 2
            if (!isTransitioning) {
                isTransitioning = true;
                setTimeout(transitionToStage2, 500);
            }
        }
    }
    requestAnimationFrame(updatePreloaderProgress);

    // Transition from Stage 1 (Logo) to Stage 2 (Cinematic Video)
    function transitionToStage2() {
        if (!preloaderStage1 || !preloaderStage2) {
            hidePreloader();
            return;
        }

        // Fade out Stage 1
        preloaderStage1.style.opacity = '0';
        preloaderStage1.style.pointerEvents = 'none';

        // Prepare Stage 2
        preloaderStage2.style.display = 'flex';
        // Force Reflow
        preloaderStage2.offsetHeight;
        preloaderStage2.style.opacity = '1';
        preloaderStage2.style.pointerEvents = 'auto';

        // Clean up particles to save CPU/GPU during video playback
        if (cleanupParticles) cleanupParticles();

        if (preloaderVideo) {
            // Start muted by default to guarantee instant autoplay on all mobile browsers (no freezing)
            preloaderVideo.muted = true;
            if (preloaderAudioIcon) {
                preloaderAudioIcon.innerHTML = mutedSvg;
            }

            // Unmute setting and toggle logic
            if (preloaderAudioToggle) {
                preloaderAudioToggle.addEventListener('click', () => {
                    preloaderVideo.muted = !preloaderVideo.muted;
                    if (preloaderAudioIcon) {
                        preloaderAudioIcon.innerHTML = preloaderVideo.muted ? mutedSvg : unmutedSvg;
                    }
                });
            }

            // Sync video progress bar
            preloaderVideo.addEventListener('timeupdate', () => {
                if (preloaderVideo.duration) {
                    const videoPercent = (preloaderVideo.currentTime / preloaderVideo.duration) * 100;
                    if (preloaderVideoProgressBar) {
                        preloaderVideoProgressBar.style.width = videoPercent + '%';
                    }
                }
            });

            // Video buffering state indicators
            preloaderVideo.addEventListener('waiting', () => {
                if (preloaderVideoCaption) {
                    preloaderVideoCaption.textContent = "Cargando presentación...";
                }
            });

            preloaderVideo.addEventListener('playing', () => {
                if (preloaderVideoCaption) {
                    preloaderVideoCaption.textContent = "Presentación Institucional • Génesis";
                }
            });

            // Absolute failure callback: if video file fails to load entirely (e.g. 404 or corrupted)
            preloaderVideo.addEventListener('error', (e) => {
                console.error("Error cargando video de presentación:", e);
                hidePreloader();
            });

            // End of cinematic video triggers the main site entry
            preloaderVideo.addEventListener('ended', () => {
                hidePreloader();
            });

            // Set up a unified listener to unmute on first click/touchstart anywhere
            const setupAutoUnmute = () => {
                const unmuteOnInteraction = () => {
                    if (preloaderVideo.muted) {
                        preloaderVideo.muted = false;
                        if (preloaderAudioIcon) {
                            preloaderAudioIcon.innerHTML = unmutedSvg;
                        }
                    }
                    // Clean up listeners immediately after first interaction
                    document.removeEventListener('click', unmuteOnInteraction);
                    document.removeEventListener('touchstart', unmuteOnInteraction);
                };
                document.addEventListener('click', unmuteOnInteraction);
                document.addEventListener('touchstart', unmuteOnInteraction);
            };

            // Start playing the video (being muted initially guarantees immediate autoplay on mobile)
            const playPromise = preloaderVideo.play();
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    clearTimeout(stage2LoadFallback);
                    setupAutoUnmute();
                }).catch(error => {
                    console.log("Error de autoplay:", error);
                    // Mute and retry as absolute fallback
                    preloaderVideo.muted = true;
                    if (preloaderAudioIcon) {
                        preloaderAudioIcon.innerHTML = mutedSvg;
                    }
                    preloaderVideo.play().then(() => {
                        clearTimeout(stage2LoadFallback);
                        setupAutoUnmute();
                    }).catch(err => {
                        console.error("Incapaz de reproducir incluso silenciado:", err);
                        hidePreloader();
                    });
                });
            }




        } else {
            // If no video element exists, proceed directly to main site
            hidePreloader();
        }
    }

    // Hide preloader fully and reveal the site
    function hidePreloader() {
        // Clear safety fallback
        clearTimeout(stage2LoadFallback);

        if (preloader.classList.contains('hidden')) return;
        
        preloader.classList.add('hidden');
        document.body.style.overflow = '';
        
        // Pause video if playing
        if (preloaderVideo) {
            try {
                preloaderVideo.pause();
            } catch(e) {}
        }

        // Initialize main website animations
        initRevealAnimations();
        animateCounters();
        initIntelligenceNetwork();
        initTypewriterEffect();
    }

    // ── Navigation ─────────────────────────────────
    const nav = document.getElementById('nav');
    const navBurger = document.getElementById('navBurger');
    const mobileMenu = document.getElementById('mobileMenu');
    const navLinks = document.querySelectorAll('.nav-link, .mobile-link');

    // Scroll effect
    let lastScroll = 0;
    window.addEventListener('scroll', () => {
        const currentScroll = window.pageYOffset;
        if (currentScroll > 50) {
            nav.classList.add('scrolled');
        } else {
            nav.classList.remove('scrolled');
        }
        lastScroll = currentScroll;
    });

    // Mobile menu toggle
    navBurger.addEventListener('click', () => {
        navBurger.classList.toggle('active');
        mobileMenu.classList.toggle('active');
        document.body.style.overflow = mobileMenu.classList.contains('active') ? 'hidden' : '';
    });

    // Close mobile menu on link click
    document.querySelectorAll('.mobile-link, .mobile-cta').forEach(link => {
        link.addEventListener('click', () => {
            navBurger.classList.remove('active');
            mobileMenu.classList.remove('active');
            document.body.style.overflow = '';
        });
    });

    // Active nav link on scroll
    const sections = document.querySelectorAll('section[id]');
    function updateActiveNav() {
        const scrollY = window.pageYOffset + 200;
        sections.forEach(section => {
            const top = section.offsetTop;
            const height = section.offsetHeight;
            const id = section.getAttribute('id');
            const link = document.querySelector(`.nav-link[href="#${id}"]`);
            if (link) {
                if (scrollY >= top && scrollY < top + height) {
                    link.classList.add('active');
                } else {
                    link.classList.remove('active');
                }
            }
        });
    }
    window.addEventListener('scroll', updateActiveNav);

    // ── Smooth scroll for anchor links ─────────────
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            e.preventDefault();
            const target = document.querySelector(this.getAttribute('href'));
            if (target) {
                const offset = 80;
                const top = target.getBoundingClientRect().top + window.pageYOffset - offset;
                window.scrollTo({ top, behavior: 'smooth' });
            }
        });
    });

    // ── Reveal on scroll ───────────────────────────
    function initRevealAnimations() {
        const observerOptions = {
            threshold: 0.15,
            rootMargin: '0px 0px -50px 0px'
        };

        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('visible');
                    observer.unobserve(entry.target);
                }
            });
        }, observerOptions);

        document.querySelectorAll('.reveal-up, .reveal-scale').forEach(el => {
            observer.observe(el);
        });
    }

    // ── Counter animation (Senior Professional) ────────
    function animateCounters() {
        const counters = document.querySelectorAll('.stat-number[data-count]');
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const el = entry.target;
                    const target = parseInt(el.dataset.count);
                    const duration = 2500; // 2.5 seconds for a dynamic but smooth pace
                    
                    // Add a slight delay before starting to sync with reveal-up
                    setTimeout(() => {
                        const start = performance.now();
                        function updateCounter(timestamp) {
                            const progress = Math.min((timestamp - start) / duration, 1);
                            // Standard ease-out
                            const eased = 1 - Math.pow(1 - progress, 3);
                            
                            // Using Math.round prevents the "hanging" pause on the second-to-last number
                            let currentValue = Math.round(eased * target);
                            
                            // Ensure we don't overshoot the target due to rounding
                            if (currentValue > target) currentValue = target;
                            
                            el.textContent = currentValue;
                            
                            if (progress < 1) {
                                requestAnimationFrame(updateCounter);
                            }
                        }
                        requestAnimationFrame(updateCounter);
                    }, 600); // Wait for the card to fade up
                    
                    observer.unobserve(el);
                }
            });
        }, { threshold: 0.8 }); // Wait until more of the element is visible

        counters.forEach(counter => observer.observe(counter));
    }

    // ── Typewriter Effect ──────────────────────────
    function initTypewriterEffect() {
        const verseEl = document.querySelector('.verse-text');
        if (!verseEl) return;
        
        const text = verseEl.textContent.trim();
        verseEl.textContent = ''; // Clear it initially
        verseEl.style.opacity = 1; // Ensure container is visible
        
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    let i = 0;
                    const speed = 55; // Slower than original, but faster than 75
                    
                    // Add cursor
                    verseEl.innerHTML = '<span class="typewriter-cursor">|</span>';
                    
                    function typeWriter() {
                        if (i < text.length) {
                            // Insert character before cursor
                            verseEl.innerHTML = text.substring(0, i + 1) + '<span class="typewriter-cursor">|</span>';
                            i++;
                            // Add slight random variation to typing speed for realism
                            const randomDelay = speed + (Math.random() * 20 - 10);
                            setTimeout(typeWriter, randomDelay);
                        } else {
                            // Blinking cursor at the end
                            const cursor = document.querySelector('.typewriter-cursor');
                            if (cursor) cursor.style.animation = 'blink 1s infinite';
                        }
                    }
                    
                    setTimeout(typeWriter, 500); // Delay start slightly
                    observer.unobserve(verseEl);
                }
            });
        }, { threshold: 0.5 });
        
        observer.observe(verseEl);
    }

    // ── Intelligence Network ───────────────────────
    function initIntelligenceNetwork() {
        const nodes = document.querySelectorAll('.neural-node');
        const intelligenceCard = document.getElementById('intelligenceCard');
        const cardTitle = document.getElementById('cardTitle');
        const cardDesc = document.getElementById('cardDesc');
        const cardIcon = document.getElementById('cardIcon');
        const centralPlaceholder = document.querySelector('.central-info-placeholder');
        const connectionsContainer = document.getElementById('neuralConnections');
        const particlesContainer = document.getElementById('neuralParticles');

        const intelligenceData = {
            'ling': {
                title: 'Inteligencia Lingüística',
                desc: 'Desarrollamos la capacidad de usar el lenguaje de manera efectiva, fomentando la lectura, escritura y comunicación oral.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path></svg>'
            },
            'log': {
                title: 'Inteligencia Lógico-Matemática',
                desc: 'Potenciamos el pensamiento lógico, resolución de problemas y comprensión de patrones matemáticos.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>'
            },
            'esp': {
                title: 'Inteligencia Espacial',
                desc: 'Fomentamos la visualización, orientación espacial y creatividad visual en artes y diseño.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>'
            },
            'kin': {
                title: 'Inteligencia Kinestésica',
                desc: 'Desarrollamos la coordinación motriz, expresión corporal y aprendizaje a través del movimiento.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8h1a4 4 0 0 1 0 8h-1"></path><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"></path><line x1="6" y1="1" x2="6" y2="4"></line><line x1="10" y1="1" x2="10" y2="4"></line><line x1="14" y1="1" x2="14" y2="4"></line></svg>'
            },
            'mus': {
                title: 'Inteligencia Musical',
                desc: 'Estimulamos la sensibilidad rítmica, apreciación musical y expresión artística sonora.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg>'
            },
            'inter': {
                title: 'Inteligencia Interpersonal',
                desc: 'Fomentamos las habilidades sociales, empatía y capacidad de trabajar en equipo.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>'
            },
            'intra': {
                title: 'Inteligencia Intrapersonal',
                desc: 'Desarrollamos la autoconciencia, autorregulación emocional y pensamiento reflexivo.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>'
            },
            'nat': {
                title: 'Inteligencia Naturalista',
                desc: 'Promovemos la conexión con la naturaleza, observación científica y conciencia ambiental.',
                icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l9 4.9V17l-9 5-9-5V6.9L12 2z"></path></svg>'
            }
        };

        const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;

        function activateNode(node) {
            const intelligence = node.dataset.intelligence;
            const data = intelligenceData[intelligence];

            if (!data) return;

            // Vanguard Positioning Logic: Move towards center from node
            const network = document.querySelector('.neural-network');
            const netRect = network.getBoundingClientRect();
            const nodeRect = node.getBoundingClientRect();
            
            const x = (nodeRect.left + nodeRect.width / 2) - netRect.left;
            const y = (nodeRect.top + nodeRect.height / 2) - netRect.top;
            
            const centerX = netRect.width / 2;
            const centerY = netRect.height / 2;
            
            const dx = x - centerX;
            const dy = y - centerY;
            const dist = Math.sqrt(dx * dx + dy * dy);
            
            // The card remains dead center.
            intelligenceCard.style.setProperty('left', '50%', 'important');
            intelligenceCard.style.setProperty('top', '50%', 'important');
            intelligenceCard.style.setProperty('transform', 'translate(-50%, -50%) scale(1)', 'important');
            intelligenceCard.style.setProperty('right', 'auto', 'important');
            intelligenceCard.style.setProperty('bottom', 'auto', 'important');

            // Fade effect for text
            cardTitle.style.opacity = 0;
            cardDesc.style.opacity = 0;
            cardIcon.style.opacity = 0;

            setTimeout(() => {
                cardTitle.textContent = data.title;
                cardDesc.textContent = data.desc;
                cardIcon.innerHTML = data.icon;
                
                cardTitle.style.opacity = 1;
                cardDesc.style.opacity = 1;
                cardIcon.style.opacity = 1;
            }, 200);

            nodes.forEach(n => n.classList.remove('active'));
            node.classList.add('active');
            
            intelligenceCard.classList.add('active');
            intelligenceCard.classList.add('visible');
            if (centralPlaceholder) centralPlaceholder.classList.add('hidden');
        }

        function deactivateNetwork() {
            if (!isTouchDevice) {
                nodes.forEach(n => n.classList.remove('active'));
                intelligenceCard.classList.remove('active');
                intelligenceCard.classList.remove('visible');
                if (centralPlaceholder) centralPlaceholder.classList.remove('hidden');
            }
        }

        nodes.forEach(node => {
            if (!isTouchDevice) {
                node.addEventListener('mouseenter', () => activateNode(node));
            }

            node.addEventListener('click', (e) => {
                e.stopPropagation();
                activateNode(node);
            });
        });

        if (!isTouchDevice) {
            const networkArea = document.querySelector('.neural-network');
            if (networkArea) {
                networkArea.addEventListener('mouseleave', deactivateNetwork);
            }
        }

        // Click outside to close on mobile
        document.addEventListener('click', (e) => {
            if (isTouchDevice && !e.target.closest('.neural-node') && !e.target.closest('.intelligence-card-premium')) {
                nodes.forEach(n => n.classList.remove('active'));
                intelligenceCard.classList.remove('visible');
                if (centralPlaceholder) centralPlaceholder.classList.remove('hidden');
            }
        });

        // Dynamic SVG Connections
        function createConnections() {
            if (!connectionsContainer) return;
            connectionsContainer.innerHTML = '';
            
            const centerX = 500;
            const centerY = 500;
            const rx = 520;
            const ry = 280;

            nodes.forEach((node, i) => {
                const angle = (i * 45) * (Math.PI / 180);
                const x = centerX + rx * Math.cos(angle);
                const y = centerY + ry * Math.sin(angle);

                const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
                line.setAttribute('x1', centerX);
                line.setAttribute('y1', centerY);
                line.setAttribute('x2', x);
                line.setAttribute('y2', y);
                line.setAttribute('stroke', 'url(#lineGrad)');
                line.setAttribute('stroke-width', '1');
                line.setAttribute('class', 'neural-line');
                connectionsContainer.appendChild(line);
            });
        }

        // Neural Particles
        function createParticles() {
            if (!particlesContainer) return;
            for (let i = 0; i < 20; i++) {
                const p = document.createElement('div');
                p.className = 'neural-particle';
                p.style.left = Math.random() * 100 + '%';
                p.style.top = Math.random() * 100 + '%';
                p.style.animationDelay = Math.random() * 5 + 's';
                p.style.opacity = Math.random() * 0.5;
                particlesContainer.appendChild(p);
            }
        }

        createConnections();
        createParticles();
    }

    // ── Smooth section transitions ─────────────────
    const sectionObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.style.opacity = '1';
                entry.target.style.transform = 'translateY(0)';
            }
        });
    }, { threshold: 0.1 });

    document.querySelectorAll('.section').forEach(section => {
        section.style.opacity = '0';
        section.style.transform = 'translateY(20px)';
        section.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
        sectionObserver.observe(section);
    });

    // ── Dynamic year in footer ─────────────────────
    const yearSpan = document.querySelector('.footer-year');
    if (yearSpan) yearSpan.textContent = new Date().getFullYear();

    // ── Keyboard navigation ────────────────────────
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && mobileMenu.classList.contains('active')) {
            navBurger.classList.remove('active');
            mobileMenu.classList.remove('active');
            document.body.style.overflow = '';
        }
    });
});
